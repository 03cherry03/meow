...
private:
    ...
    // void fusePoses()gnss_pos
    // {
    //     // Check if GNSS or LiDAR is used exclusively
    //     Eigen::Vector3d lidar_pos;
    //     Eigen::Vector3d gnss_pos;

    //     // Handle GNSS exclusively (weight_1.0, weight_0.0 case)
    //     if (gnss_weight_ > 0.0 && lidar_weight_ <= 0.0)
    //     {
    //         gnss_pos = convertGnssToUTM(last_gnss_msg_->pose.pose.position);
    ...
    //         fused_pose.pose.pose.orientation = last_gnss_msg_->pose.pose.orientation;

    //         // Use GNSS covariance
    //         fused_pose.pose.covariance = last_gnss_msg_->pose.covariance;

    //         final_pose_pub_->publish(fused_pose);
    //     }
    //     // Handle LiDAR exclusively (weight_0.0, weight_1.0 case)
    //     else if (lidar_weight_ > 0.0 && gnss_weight_ <= 0.0)
    //     {
    //         lidar_pos = Eigen::Vector3d(last_lidar_msg_->pose.pose.position.x, 
    //                                     last_lidar_msg_->pose.pose.position.y, 
    //                                     last_lidar_msg_->pose.pose.position.z);

    //         geometry_msgs::msg::PoseWithCovarianceStamped fused_pose;
    //         fused_pose.header.stamp = this->now();
    //         fused_pose.header.frame_id = "base_link";

    //         fused_pose.pose.pose.position.x = lidar_pos.x();
    //         fused_pose.pose.pose.position.y = lidar_pos.y();
    //         fused_pose.pose.pose.position.z = lidar_pos.z();
    //         fused_pose.pose.pose.orientation = last_lidar_msg_->pose.pose.orientation;

    //         // Use LiDAR covariance
    //         fused_pose.pose.covariance = last_lidar_msg_->pose.covariance;

    //         final_pose_pub_->publish(fused_pose);
    //     }
    //     // Handle both LiDAR and GNSS (default case)
    //     else
    //     {
    //         lidar_pos = Eigen::Vector3d(last_lidar_msg_->pose.pose.position.x, 
    //                                     last_lidar_msg_->pose.pose.position.y, 
